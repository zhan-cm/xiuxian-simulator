...
def _classify(action: str, output: str) -> tuple[str, str, str]:
    ...
    if any(action_text.startswith(word) for word in ("情缘", "情劫", "人脉", "缘网", "众生缘网", "介入人情", "对话", "交谈", "送礼", "论道", "结为道侣", "双修", "确立关系", "回应", "护道")):
    ...
    if any(action_text.startswith(word) for word in ("拍卖", "竞拍", "天机拍卖")):
        return "槌", "trade", "天机竞价"
    if any(action_text.startswith(word) for word in ("修炼", "闭关", "参悟", "参研道法", "研习功法", "研习法术", "悟道", "观想", "消化感悟", "点亮")):
    ...
    if action_text in {"主线", "因果", "主线卷宗", "推进主线"} or action_text.startswith("主线选择"):
        return "潮", "story", "灵潮因果"
    if any(action_text.startswith(word) for word in ("宗门", "加入", "宗门任务", "藏经阁", "兑换传承", "宗门传功", "申请晋升", "宗门大比", "护宗战", "驰援前线", "固守山门", "闭关不出", "叛宗", "确认叛宗")):
        return "宗", "sect", "宗门因果"
