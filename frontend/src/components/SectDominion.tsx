...
import { BookOpenText, ChevronRight, CircleDollarSign, Crown, History, Landmark, LockKeyhole, Shield, Sparkles, UserPlus, UsersRound, X } from 'lucide-react'
import type { SectDomainSnapshot } from '../api/types'
...
export function SectDominion({ domain, busy, readOnly = false, onAction }: Props) {
  ...
  return <Dialog.Root>
    <Dialog.Trigger asChild>
      <button className="domain-ribbon" type="button" data-founded={domain.founded || undefined} data-pending={domain.pending || undefined}>
        <span>{domain.founded ? <Crown size={17} /> : <Landmark size={17} />}</span>
        <div><small>{domain.founded ? sect.level_name : '金丹之后 · 一方之主'}</small><strong>{domain.founded ? sect.name : domain.pending ? '择定开山道统' : '开宗立派'}</strong></div>
        <p>{domain.founded ? `${domain.disciples.length} 名门人 · 月净 ${(sect.monthly_net || 0) >= 0 ? '+' : ''}${sect.monthly_net || 0}` : domain.pending ? `${domain.suggested_name}等待道统` : domain.found_reason || '资粮与声名俱足'}</p>
        <em>{domain.founded ? `${sect.treasury} 库藏` : domain.requirements.filter((item) => item.met).length + '/4'}</em><ChevronRight size={16} />
      </button>
    </Dialog.Trigger>
    <Dialog.Portal>
      ...
      <Dialog.Content className="character-dialog domain-dialog">
        <header>
          <div><p>{domain.founded ? '传道授业 · 山门长青' : '聚众传法 · 自立道统'}</p><Dialog.Title>{domain.founded ? sect.name : '开宗立派'}</Dialog.Title><Dialog.Description>{domain.founded ? '门人、库藏、道统与山门设施会随九州月份真实演化。' : '达到金丹、积累声望与资粮后，可亲立山门并从三种根本道统中择一。'}</Dialog.Description></div>
...
function Dominion({ domain, busy, readOnly, onAction }: Props) {
  const sect = domain.sect
  return <div className="domain-scroll">
    ...
    <div className="domain-growth"><span><i><b style={{ width: `${sect.experience_percent || 0}%` }} /></i><small>宗门底蕴 {sect.experience}/{sect.experience_required}</small></span><em>月度收支 {(sect.monthly_net || 0) >= 0 ? '+' : ''}{sect.monthly_net || 0}</em></div>
